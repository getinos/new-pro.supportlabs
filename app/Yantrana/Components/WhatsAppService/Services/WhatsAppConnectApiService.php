...
class WhatsAppConnectApiService extends BaseEngine implements WhatsAppServiceEngineInterface
{
    ...
    public function processEmbeddedSignUp($request)
    {
        ...
        if(!$phoneNumberId) {
            ...
            // Detect if this is WhatsApp Business App Coexistence onboarding
            $isWhatsAppBusinessAppOnboarding = isset($phoneNumberRecord['is_on_biz_app']) && $phoneNumberRecord['is_on_biz_app'] === true;
            
            // if the record not found we may need to register this
            // SKIP REGISTRATION for WhatsApp Business App Coexistence (number already registered)
            if($phoneNumberRecord['platform_type'] != 'CLOUD_API' && !$isWhatsAppBusinessAppOnboarding) {
                updateProgressTextModel(
            ...
            } elseif ($isWhatsAppBusinessAppOnboarding) {
                updateProgressTextModel(
                    __tr('WhatsApp Business App detected - skipping phone registration ...')
                );
            }
        }
        ...
        $dataToUpdate = [
            ...
            'whatsapp_onboarding_raw_data' => [
                ...
                'is_whatsapp_business_app_onboarding' => $isWhatsAppBusinessAppOnboarding ?? false,
                'onboarding_type' => ($isWhatsAppBusinessAppOnboarding ?? false) ? 'coexistence' : 'standard',
            ]
        ];
        
        // If this is WhatsApp Business App onboarding, initiate sync immediately
        if ($isWhatsAppBusinessAppOnboarding ?? false) {
            updateProgressTextModel(
                __tr('initiating WhatsApp Business App data synchronization ...')
            );
            // Store flag to trigger sync after setup completes
            $dataToUpdate['smb_sync_pending'] = true;
            $dataToUpdate['smb_sync_initiated_at'] = now();
        }
    ...
    public function getPhoneNumbers($wabaId, $accessToken = null)
    {
        ...
        // Include is_on_biz_app and platform_type for coexistence detection
        return $this->apiGetRequest("{$this->baseApiRequestEndpoint}{$wabaId}/phone_numbers?fields=display_phone_number,certificate,name_status,new_certificate,new_name_status,last_onboarded_time,is_on_biz_app,platform_type", []) ?? null;
    }
    
    /**
     * Initiate SMB contacts synchronization
     *
     * @param string $phoneNumberId
     * @return array
     * @link https://developers.facebook.com/docs/whatsapp/business-management-api/guides/coexistence#synchronizing-whatsapp-business-app-data
     */
    public function syncSmbContacts($phoneNumberId)
    {
        updateProgressTextModel(
            __tr('Syncing WhatsApp Business App contacts ...')
        );
        
        return $this->apiPostRequest("{$this->baseApiRequestEndpoint}{$phoneNumberId}/smb_app_data", [
            'messaging_product' => 'whatsapp',
    ...
    /**
     * Initiate SMB message history synchronization
     *
     * @param string $phoneNumberId
     * @return array
     * @link https://developers.facebook.com/docs/whatsapp/business-management-api/guides/coexistence#synchronizing-whatsapp-business-app-data
     */
    public function syncSmbHistory($phoneNumberId)
    {
        updateProgressTextModel(
            __tr('Syncing WhatsApp Business App message history ...')
        );
