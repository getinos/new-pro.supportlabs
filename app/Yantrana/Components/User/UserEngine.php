...
class UserEngine extends BaseEngine implements UserEngineInterface
{
    ...
    public function prepareUserDataTableSource()
    {
        ...
        $requireColumns = [
            ...
            'first_name',
            'last_name',
            'email', // Removed username as email is now used as username
            'mobile_number',
    ...
    /**
      ...
      * @return  array
      *---------------------------------------------------------------- */
    public function processUserUpdate($userIdOrUid, $inputData)
    {
        // Convert input data to array if it's an object to fix the TypeError
        ...
        // Check if $user not exist then throw not found exception
        if (__isEmpty($user)) {
            return $this->engineResponse(18, null, __tr('User not found.'));
        }
        ...
        if ($validator->fails()) {
            return $this->engineFailedResponse($validator->errors(), __tr('Validation errors'));
        }
        ...
        $updateData = [
            ...
            'username' => $inputData['email'], // Set username as email
