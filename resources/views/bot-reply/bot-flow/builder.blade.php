...
<script>
    ...
    window.updateDraft = function(response) {
        ...
        // Update delete and duplicate button visibility
        window.updateOperatorButtonsVisibility();
        return true;
    };
    
    // Function to update button visibility based on unsaved content state
    window.updateOperatorButtonsVisibility = function() {
        const isUnsaved = window.__isUnsavedContent || false;
        // Find all delete buttons in operator bodies - look for buttons with delete functionality
        $('#lwBotFlowBuilder').find('a[data-callback="onBotReplyDeleted"]').each(function() {
            const $deleteBtn = $(this);
            const $btnGroup = $deleteBtn.closest('.btn-group');
            // Find the unsaved delete button in the same group
            const $unsavedDeleteBtn = $btnGroup.find('a[onclick*="unsavedAlert"]').filter(function() {
                return $(this).find('i.fa-trash').length > 0;
            });
            if (isUnsaved) {
                $deleteBtn.css('display', 'none');
                $unsavedDeleteBtn.css('display', 'inline-block');
            } else {
                $deleteBtn.css('display', 'inline-block');
                $unsavedDeleteBtn.css('display', 'none');
            }
        });
        // Find all duplicate buttons in operator bodies
        $('#lwBotFlowBuilder').find('a[data-confirm="#lwDuplicateBotReply-template"]').each(function() {
            const $duplicateBtn = $(this);
            const $btnGroup = $duplicateBtn.closest('.btn-group');
            // Find the unsaved duplicate button in the same group
            const $unsavedDuplicateBtn = $btnGroup.find('a[onclick*="unsavedAlert"]').filter(function() {
                return $(this).find('i.fa-copy').length > 0;
            });
            if (isUnsaved) {
                $duplicateBtn.css('display', 'none');
                $unsavedDuplicateBtn.css('display', 'inline-block');
            } else {
                $duplicateBtn.css('display', 'inline-block');
                $unsavedDuplicateBtn.css('display', 'none');
            }
        });
    };
    window.saveFlowChartData = function() {
        window.isFlowChatInitialized = false;
        let flowChartData = window.$flowBuilderInstance.flowchart('getData');

        // Enrich operators with flow_message_reply payload when available from window.__flowBotsMap
        if (flowChartData && flowChartData.operators && window.__flowBotsMap) {
            Object.keys(flowChartData.operators).forEach(function(opId) {
                const op = flowChartData.operators[opId];
                if (!op || !op.properties) return;
                const bot = window.__flowBotsMap[opId];
                const flowData = bot ? _.get(bot, '__data.flow_message') : null;
                if (flowData) {
                    op.properties.type = 'flow_message_reply';
                    // carry a minimal payload so backend sees flow data
                    op.properties.flow_message = flowData;
                    // also adjust UI body to reflect flow type
                    if (op.properties.body && op.properties.body.includes('Simple message')) {
                        const flowName = flowData.flow_name || 'WhatsApp Flow';
                        op.properties.body = `<div style="padding: 8px; background: #f0fdfa; border-left: 3px solid #0ea5e9; margin-bottom: 8px;">
                            <strong style="color: #0ea5e9; font-size: 12px;">FLOW MESSAGE</strong><br>
                            <span style="color: #0f172a; font-size: 13px; font-weight: 600;">${flowName}</span>
                        </div>`;
                    }
                }
            });
        ...
        __DataRequest.post("{{ route('vendor.bot_reply.bot_flow_data.write.update') }}", {
            ...
            'flow_chart_data' : flowChartData
                ...
                window.__isUnsavedContent = false;
                // Update button visibility after save
                window.updateOperatorButtonsVisibility();
                // __Utils.viewReload();
        });
    };
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('initialAlpineData', () => ({
            ...
            processedFlowBots: function () {
                ...
                var nodeCounter = 1;
                // Ensure flowBots is treated as an array for proper iteration
                const flowBotsArray = Array.isArray(this.flowBots) ? this.flowBots : Object.values(this.flowBots);
                window.__flowBotsMap = {};
                for (let i = 0; i < flowBotsArray.length; i++) {
                    const element = flowBotsArray[i];
                        window.__flowBotsMap[element._uid] = element;
                        let nodeType = 'message'; // Default to message

                        // Enhanced node type detection
                        if(element.type === 'flow_message_reply' || _.get(element.__data, 'flow_message')) {
                        ...
                        if (nodeType === 'wait') {
                        ...
                        } else if (nodeType === 'flow_message_reply') {
                            ...
                            const bodyText = flowData.body_text || element.reply || "{{ __tr('Body not configured') }}";
                        ...
                        const isUnsaved = window.__isUnsavedContent || false;
                        const deleteUrl = __Utils.apiURL("{{ route('vendor.bot_reply.write.delete', [ 'botReplyIdOrUid']) }}", {'botReplyIdOrUid': element._uid});
                        const duplicateUrl = __Utils.apiURL("{{ route('vendor.bot_reply.write.duplicate', [ 'botReplyIdOrUid']) }}", {'botReplyIdOrUid': element._uid});
                        const originalButtons = `<div class="btn-group btn-group-sm">
                            ...
                            <a style="display:${isUnsaved ? 'inline-block' : 'none'};" onclick="window.unsavedAlert(); return false;" title="{{  __tr('Delete') }}" class="btn btn-danger btn-sm" href="#"><i class="fa fa-trash"></i> {{  __tr('Delete') }} </a>
                            <a style="display:${isUnsaved ? 'none' : 'inline-block'};" data-method="post" href="`+deleteUrl+`" class="btn btn-danger lw-ajax-link-action" data-confirm="#lwDeleteBotReply-template" title="{{ __tr('Delete') }}" data-callback="onBotReplyDeleted"><i class="fa fa-trash"></i> {{  __tr('Delete') }}</a> 
                            <a style="display:${isUnsaved ? 'inline-block' : 'none'};" onclick="window.unsavedAlert(); return false;" title="{{  __tr('Duplicate') }}" class="btn btn-light btn-sm" href="#"><i class="fa fa-copy"></i></a> 
                           <a style="display:${isUnsaved ? 'none' : 'inline-block'};" data-method="post" href="`+duplicateUrl+`" class="btn btn-light lw-ajax-link-action" data-confirm="#lwDuplicateBotReply-template" data-post-data='{"context_flow_uid": "{{ $botFlow->_uid }}"}' title="{{ __tr('Duplicate') }}"><i class="fa fa-copy"></i></a>
                            <a class="btn btn-light" style="padding: 1px;"></a>
                        </div>
                        <button style="display:none;" class="lw-delete-link-btn lw-operator-link-`+element._uid+` btn btn-warning btn-block btn-sm" onclick="window.$flowBuilderInstance.flowchart('deleteSelected'); return false;"><i class="fas fa-unlink"></i> {{  __tr('Delete Link') }}</button>`;
                        ...
                        data.operators[element._uid] = {
                            ...
                            properties: {
                                ...
                                flow_message: (nodeType === 'flow_message_reply') ? (element.__data?.flow_message || {}) : undefined,
                                inputs: {
                                    input: {
                                        label: "-->"
                                    },
                ...
                if(window.$flowBuilderInstance) {
                    ...
                    setTimeout(function() {
                        // Initialize button visibility based on current unsaved state
                        window.updateOperatorButtonsVisibility();
                        ...
                        // Add handler ONLY for EDIT buttons - let delete/duplicate use default behavior
                        $('#lwBotFlowBuilder').on('click', 'a.lw-ajax-link-action[data-response-template="#lwEditBotReplyBody"]', function(e) {
                            var $link = $(this);
                            
                            // Stop propagation for edit buttons only
                            ...
                            if ($link.data('pre-callback') === 'appFuncs.clearContainer' && 
                                typeof window.appFuncs !== 'undefined' && 
                                typeof window.appFuncs.clearContainer === 'function') {
                        ...
                        // Separate handler for delete/duplicate buttons - prevent default but allow global handler
                        $('#lwBotFlowBuilder').on('click', 'a.lw-ajax-link-action[data-callback="onBotReplyDeleted"], a.lw-ajax-link-action[data-confirm="#lwDuplicateBotReply-template"]', function(e) {
                            // Prevent default link behavior (GET request)
                            e.preventDefault();
                            e.stopImmediatePropagation(); // Prevent other handlers
                            e.stopPropagation(); // Stop propagation to flowchart
                            
                            // Manually trigger the global handler since we stopped propagation
                            var $link = $(this);
                            
                            // Debug: verify data-method is set
                            console.log('Delete/Duplicate button clicked', {
                                href: $link.attr('href'),
                                dataMethod: $link.data('method'),
                                attrMethod: $link.attr('data-method'),
                                hasConfirm: $link.data('confirm')
                            });
                            
                            var confirmMessage = $link.data('confirm');
                            if (confirmMessage) {
                                showConfirmation(confirmMessage, function () {
                                    return __DataRequest.process($link);
                                });
                            } else {
                                return __DataRequest.process($link);
                            }
                        ...
                        // Prevent flowchart from selecting operator when clicking on interactive elements
                        $('#lwBotFlowBuilder').on('click', '.flowchart-operator', function(e) {
                            // Check if this is an interactive element click
                            ...
                            if ($target.is('a, button, input, select, textarea') || 
                                ...
                                e.stopPropagation(); // Only stop propagation to parent, not immediate
                                return false;
                            }
                        });
                    }, 300);
